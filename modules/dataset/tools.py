...
def prepare_dataset(batch_size, transform, drop_last = None, num_workers = 0):
    '''
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=drop_last, num_workers=num_workers)

    # test data loader
    test_dataset = Image_Dataset(
        images=dataset['test']['img'], transform=transform)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True, drop_last=drop_last, num_workers=num_workers)
